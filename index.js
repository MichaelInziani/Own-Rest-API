...
app.get("/random", async (req, res) => {
    try {
        const randomIndex = await Math.floor(Math.random() * jokes.length);
...
app.get("/jokes/:id", async (req, res) => {
    try {
        ...
        const foundJoke = await jokes.find((joke) => joke.id === id);
        res.json(foundJoke);
    } catch (error) {
        console.log(error.message);
    }
...
app.get("/filter", async (req, res) => {
    try {
        ...
        const filteredActivities = await jokes.filter((joke) => joke.jokeType === type);
